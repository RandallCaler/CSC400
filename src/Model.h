...
private:
    ...
    Mesh processMesh(aiMesh *mesh, const aiScene *scene)
    {
        ...
        // 3. normal maps
        std::vector<shared_ptr<Texture>> normalMaps = loadMaterialTextures(material, aiTextureType_HEIGHT, "texture_normal", scene);
        textures.insert(textures.end(), normalMaps.begin(), normalMaps.end());
        // 4. height maps
        std::vector<shared_ptr<Texture>> heightMaps = loadMaterialTextures(material, aiTextureType_AMBIENT, "texture_height", scene);
    ...
    vector<shared_ptr<Texture>> loadMaterialTextures(aiMaterial* mat, aiTextureType type, string typeName, const aiScene* scene)
    {
        ...
        for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
        {
            ...
            if (str.data[0] == '*') {   
                // Embedded texture
                std::string indexStr = std::string(str.data + 1);
    ...
    void measure()
    {
        ...
        for (auto& mesh : meshes) 
        {
            if (mesh.min.x < minX) minX = mesh.min.x;
            if (mesh.max.x > maxX) maxX = mesh.max.x;

            if (mesh.min.y < minY) minY = mesh.min.y;
            if (mesh.max.y > maxY) maxY = mesh.max.y;

            if (mesh.min.z < minZ) minZ = mesh.min.z;
            if (mesh.max.z > maxZ) maxZ = mesh.max.z;
        }
